...
def _find_free_cidr_machine(start_cidr_machine):
    ...
    routes = utils.run_command('/usr/sbin/ip route show')
    while _is_cidr_machine_exist(cidr_machine, routes):
        cidr_machine += 1
    return str(cidr_machine)


def _is_cidr_machine_exist(cidr_machine, routes):
    if str(cidr_machine) in routes:
        return True
    return _ping(str(cidr_machine.ip + 1))
...
def _find_free_network_bridge():
    links = utils.run_command(f'/usr/sbin/ip link show', shell=False)
    i = 0
    while f'tt{i}' in links:
        i += 1
    return f'tt{i}'
